...
import com.util.MetadataLoginUtil;
import com.util.common.LoadSFConfigurations;

/**
 * Deploy a zip file of metadata components. Prerequisite: Have a deploy.zip
 * file that includes a package.xml manifest file that details the contents of
 * the zip file.
 */
public class DeployService {
	...
	// String USERNAME = "sri.desti@gmail.com";
	// String PASSWORD = "infrascape34hUL9e3q8lE9m32gslQry8zMw";
	// String URL = "https://login.salesforce.com/services/Soap/c/33.0";

	public static void main(String[] args) throws Exception {

		// DeployService sample = new DeployService(USERNAME, PASSWORD, URL);
		// sample.deployZip();
	}

	public DeployService(LoadSFConfigurations loadSFConfig, String toServer)
			throws ConnectionException {
		if (loadSFConfig != null) {
			...
			if (Constants.SERVER.equals(toServer)) {
				...
				//this.metadataConnection = MetadataLoginUtil.login();
				this.metadataConnection = MetadataLoginUtil.login(
						loadSFConfig.getClientUsername(),
						loadSFConfig.getClientPassword(),
						loadSFConfig.getClientURL());
			}
		}
	}

	public void deployZip(String instance) throws RemoteException, Exception {
		...
		System.out.println("The file " + ZIP_FILE
				+ " was successfully deployed for " + instance);
	}
	...
	private void createMetadataConnection(final String username,
		...
		LoginResult loginResult = (new EnterpriseConnection(loginConfig))
