...
public class GetPackagesTask implements Runnable {
	...
	public GetPackagesTask(String orgId, String token, String serverURL,
	...
	public void run() {
		...
		try {
			...
			rSvc.initiate();
			/*GetPackageRequest request = new GetPackageRequest(getOrgId(),
					getToken(), getServerURL(), getRefreshToken(),Constants.BaseOrgTypeID,
					getReleaseId(), getReleaseName(), getReleaseStatus(),
					getMetadataLogId());

			ProcessTrack processTrack = new ProcessTrack(
					new ProcessStatus("GetPackageAppService",
							Constants.INITIATED_STATUS, "begin"));
			GetPackageResponse response = new GetPackageResponse(
					processTrack);

			GetPackageAppService appService = new GetPackageAppService(request, response);
			appService.initiate();*/
		} catch (Exception e) {
			errorFlag = true;
